...
3. **Creating TaskPred Objects:** The `Predecessors` collection's `add` method (`xer_parser/model/predecessors.py`) receives the raw data (`params`), creates a `TaskPred` object (`xer_parser/model/classes/taskpred.py`) from it, and appends it to its internal list (`self.task_pred`).

    ```python
    # Inside xer_parser/model/predecessors.py, in the Predecessors class add method
...
4. **Finding Related Relationships:** When you call `xer.relations.get_successors(task_id)` or `xer.relations.get_predecessors(task_id)`, the `Predecessors` collection object searches its internal list (`self.task_pred`) to find all `TaskPred` objects where either `pred_task_id` (for successors) or `task_id` (for predecessors) matches the requested `task_id`.
    ...
    ```python
    # Inside xer_parser/model/predecessors.py (simplified methods)
