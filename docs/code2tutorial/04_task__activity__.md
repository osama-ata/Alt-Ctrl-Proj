...
The `self._tasks` attribute is an instance of the `Tasks` collection class (`xer_parser/model/tasks.py`). Its `add` method is responsible for creating the actual `Task` object:
...
Finally, let's look inside the `Task` class itself (`xer_parser/model/classes/task.py`). The `__init__` method receives the `params` dictionary and the `data` reference:

```python
# Inside xer_parser/model/classes/task.py, in the Task class __init__
class Task:
