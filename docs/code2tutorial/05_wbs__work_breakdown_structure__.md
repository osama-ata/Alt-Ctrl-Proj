...
2. **Storing WBS Objects:** The `Reader` adds each newly created `WBS` object to the `_wbss` attribute, which is an instance of the `WBSs` collection class (`xer_parser/model/wbss.py`). This collection holds *all* WBS objects loaded from the file.
3. **WBS Object Structure:** Each `WBS` object (`xer_parser/model/classes/wbs.py`) stores its raw data attributes (like `wbs_id`, `wbs_name`, `parent_wbs_id`, `proj_id`). Crucially, it also stores a reference back to the main data container (`self.data`), which holds references to *all* the collections, including `xer.activities` (which is `self.data.tasks` internally).
...
You can refer to the `WBS` class source code (`xer_parser/model/classes/wbs.py`) for the complete list of attributes available.
